    ...
    versions.forEach((v) => {
      ...
      if(targetBranch === 'main'){
        ...
        // once (it is same for both locales)
          if (l.id === 'zh-cn') {
              let fromSidebarPath = path.join(tempDir, "docs/docusaurus/sidebars.json");
              let toSidebarPath = path.join(__dirname, "versioned_sidebars/version-" + v.branch + "-sidebars.json");
              let fromTranslationFilePath = path.join(tempDir, "docs/docusaurus/i18n/zh/docusaurus-plugin-content-docs/current.json");
              let toTranslationFilePath = path.join(__dirname,
                  `i18n/zh/docusaurus-plugin-content-docs/version-${v.branch}.json`);
          console.log(
            "\n fromSidebarPath: " , fromSidebarPath,
              "\n toSidebarPath: ", toSidebarPath,
